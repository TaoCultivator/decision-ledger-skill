...
description: Maintain and retrieve compact project decision ledgers, historical experience memory, rejected approaches, and relevant skills/CLI tools for implementation discussions, tool errors, publishing workflows, post-feedback corrections, and specification updates. Use before discussing or changing project behavior when the user mentions 沟通纪要, 废案, 方案复盘, 规范沉淀, 不要重犯, 检索历史结论, 历史经验, 记忆, experience memory, tool error, publish failure, skill routing, CLI routing, tool selection, self-iteration, self-reflection, self-audit, 自我反思, implementation discussion, rejected approach, or decision log.
---
...
- Treat rejected approaches and historical experience records as guardrails. If a rejected or experience record matches the current problem, surface it before proposing a similar path.
...
- Actively self-audit substantial turns for missed ledger hits, missed historical experience, bad skill/CLI routing, context waste, weak validation, and unclear accepted/rejected boundaries; do not wait for the user to find every gap.
...
- `references/experience-memory-template.md` for repeatable operational lessons, tool errors, environment issues, publishing failures, and workflow fixes.
- `references/publishing-workflow-experience.md` when maintaining this skill public mirror or debugging Windows/Git/PowerShell skill publishing failures.
...
1. Extract query keys from the user request: project, feature, module, file path, function name, command name, tool name, error text, symptom, metric, sample name, decision status, and exact terms.
2. Locate the project ledger. Start with `docs/decision-ledger/index.jsonl`; if absent, search for `decision-ledger`, `DECISION_INDEX`, `rejected`, `accepted`, `experience`, `经验`, `记忆`, `沟通纪要`, and `废案`.
...
6. Rank results in this order: matching experience records for operational/tool failures, rejected records, accepted records, proposed records, research reviews.
...
Use this before step-by-step troubleshooting of tool, CLI, filesystem, publishing, validation, or environment failures:

1. Extract exact error strings, command names, tool names, paths, exit codes, and observable symptoms.
2. Search compact experience entries first with exact keywords. Prefer `status=experience` when the search tool supports it.
3. If a strong experience match exists, treat it as the first hypothesis, not as unquestioned truth. Verify the current conditions with cheap read-only checks before applying the remembered fix.
4. Apply the remembered resolution only when the trigger signals and constraints still match. Avoid replaying destructive commands from memory.
5. If no experience record matches, continue normal diagnosis. After resolution, create an experience record with the exact trigger, root cause, fix, validation evidence, and do-not-repeat rule.
6. Prefer specific operational memory over broad reasoning when the error is repeated: Git safe.directory, git identity, Windows encoding, PowerShell copy behavior, generated caches, credential/network failures, and release/publish steps are all experience-worthy.

Experience memory is human-like recall with evidence. It exists to avoid re-solving known failures, but it must still follow the retrieval, verification, and approval rules above.
...
2. Check the active skill list or host-provided skill metadata first. Match by 
ame` and `description`; do not open every `SKILL.md`.
...
- Include exact technical anchors when they matter: file paths, function names, command names, tool names, parameter names, sample names, exit codes, error text, and user-facing terms.
- Include synonyms and Chinese wording that future users are likely to search for.
- Change `status` deliberately: `proposed` for discussion, `accepted` after validation, `rejected` after a failed or imperfect approach is identified, and `experience` after a repeatable operational lesson has evidence.
...
- Search a compact index before reading full records, including experience memory before troubleshooting repeated operational errors.
...
- Ask whether a relevant experience/rejected/accepted record was missed, whether a better skill or CLI should have been used, whether context loading was broader than necessary, whether validation exposed a weak rule, and whether the current discussion reveals a reusable improvement.
...
1. Build and check: implement the requested workflow, then compare it against the current conversation, project rules, and relevant experience memory.
2. Self-reflect and tighten: look for missed experience records, ledger hits, rejected approaches, skill/CLI routing gaps, over-broad context loading, validation weaknesses, omissions, ambiguity, and scope creep; verify proposed improvements through the skill workflow instead of relying on intuition.
